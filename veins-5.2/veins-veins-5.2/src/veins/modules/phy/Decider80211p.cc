...
enum Decider80211p::PACKET_OK_RESULT Decider80211p::packetOk(double sinrMin, double snrMin, int lengthMPDU, double bitrate)
{
    ...
    // 添加，记录CSR的数值和次数
    SumofCSR += packetOkSinr;
    CSR_Times++;
    sinr_re = sinrMin;

    // check if header is broken
    double headerNoErrorSinr = NistErrorRate::getChunkSuccessRate(PHY_HDR_BITRATE, BANDWIDTH_11P, sinrMin, PHY_HDR_PLCPSIGNAL_LENGTH);
    ...
    if (collectCollisionStats) {
        ...
        ASSERT(headerNoErrorSnr >= headerNoErrorSinr);
    }
    ...
    if (!collectCollisionStats) {
        if (rand > headerNoErrorSinr) return NOT_DECODED;
    }
    else {

        if (rand > headerNoErrorSinr) {
...
//   此处为添加，记录CSR相关参数
    phy->recordScalar("CSR_meanValue",  SumofCSR/CSR_Times);
    phy->recordScalar("CSR",  SumofCSR);
    phy->recordScalar("CSR_times",  CSR_Times);
    phy->recordScalar("CSR_sinr",  sinr_re);
//   此处为添加，记录多用途观察量参数
    phy->recordScalar("SPY_1",  spy_1);
    phy->recordScalar("SPY_2",  spy_2);
