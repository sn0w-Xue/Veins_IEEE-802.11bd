...
double NistErrorRate::get256QamBer(double snr)
{
    double z = std::sqrt(snr / (2.0 * 85.0));
    double ber = 15.0 / 4.0 / 16.0 * erfc(z);
    return ber;
}
...
double NistErrorRate::getFec256QamBer(double snr, uint32_t nbits, uint32_t bValue)
{
...
//        return getFecBpskBer(snr_mW, nbits, 1);
        return getFecQpskBer(snr_mW + 0.9, nbits, 1);
        break;
//        此种数据传输模式不再使用
...
//        break;
    case MCS::ofdm_qpsk_r_1_2:
        return getFecQpskBer(snr_mW - 2.1, nbits, 1);
        break;
    case MCS::ofdm_qpsk_r_3_4:
        return getFecQpskBer(snr_mW - 1, nbits, 3);
        break;
    case MCS::ofdm_qam16_r_1_2:
//        return getFec16QamBer(snr_mW, nbits, 3);
        return getFecQpskBer(snr_mW - 8.4, nbits, 1);
        break;
    case MCS::ofdm_qam16_r_3_4:
//        return getFec16QamBer(snr_mW, nbits, 3);
        return getFecQpskBer(snr_mW - 7.8, nbits, 3);
        break;
    case MCS::ofdm_qam64_r_2_3:
//        return getFec64QamBer(snr_mW, nbits, 2);
        return getFecQpskBer(snr_mW - 13, nbits, 3);
        break;
    case MCS::ofdm_qam64_r_3_4:
//        return getFec64QamBer(snr_mW, nbits, 3);
        return getFecQpskBer(snr_mW - 14.3, nbits, 3);
        break;
    case MCS::ofdm_qam64_r_5_6:
//        尚未有对应参考
        return getFec64QamBer(snr_mW, nbits, 5);
        break;
    case MCS::ofdm_qam256_r_3_4:
//        尚未有对应参考
        return getFec256QamBer(snr_mW, nbits, 3);
        break;
    case MCS::ofdm_qam256_r_5_6:
//        尚未有对应参考
        return getFec256QamBer(snr_mW, nbits, 5);
        break;
    default:
